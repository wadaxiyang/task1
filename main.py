...
def boxSelection(img_hsv_get, img_get):
    img_gray = cv2.cvtColor(img_hsv_get, cv2.COLOR_BGR2GRAY)
    retval, output = cv2.threshold(img_gray, 0, 255, cv2.THRESH_BINARY)
    cv2.imshow('fg', output)
    ...
    cv2.putText(img_get, str((box[2][0] - box[0][0]) / 2 + box[0][0]) + ',' + str((box[0][1] - box[1][1]) / 2 + box[1][1]),
